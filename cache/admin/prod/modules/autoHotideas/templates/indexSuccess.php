<?php
  ...
  $appliedFilters = $sf_user->getAppliedFiltersOnModule('hotideas');
...
<div id="sf_admin_container" class='{baseUrl: "<?php echo url_for1('hotideas') ?>"}'>
  ...
  <div id="list_header" class="clearfix">
    <?php
      if (count($appliedFilters))
      {
        ...
        echo link_to(__('Back to list'), 'hotideas', array('action' => 'filter'), array('query_string' => '_reset', 'method' => 'post', 'class' => 'ml10 reset'));
        echo _close('div');
      }
    ?>
  </div>
  ...
  <div id="sf_admin_header">
    <?php include_partial('hotideas/list_header', array('pager' => $pager)) ?>
  </div>

  <?php $dmListActionBar = get_partial('hotideas/list_action_bar', array('pager' => $pager, 'helper' => $helper, 'class' => 'dm_pagination_top', 'configuration' => $configuration)); ?>
  ...
  <div id="sf_admin_content" <?php if (!$pager->getNbResults()) echo 'class="no_results"'; ?>>
    ...
    <?php include_partial('hotideas/list', array('pager' => $pager, 'sort' => $sort, 'helper' => $helper)) ?>
    ...
    <div class="dm_list_global_actions clearfix">
      ...
      <div class="dm_export">
      <?php echo link_to(__('Export csv', array(), 'dm'), 'hotideas/export', array(  'class' => 'dm_sort s16 s16_export',)) ?>      </div>
      <?php endif; ?>
      ...
      <?php
      $loremizeLink = _link('@hotideas?action=loremize&nb=__DM_NB_RECORDS__')
      ->text('__DM_NB_RECORDS__')
      ->set('.ml10.dm_js_confirm')
      ->title(__('Generate %1% random %2%', array('%1%' => '__DM_NB_RECORDS__', '%2%' => __('Hotideas'))))
      ->render();
  ...
  <div id="sf_admin_footer">
    <?php include_partial('hotideas/list_footer', array('pager' => $pager)) ?>
  </div>
</div>